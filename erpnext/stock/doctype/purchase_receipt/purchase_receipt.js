...
frappe.ui.form.on("Purchase Receipt", {
	...
	refresh: function (frm) {
		if (frm.get_docfield("is_subcontracted")) {
			frm.set_df_property("is_subcontracted", "hidden", 1);
		}
		if (frm.get_docfield("apply_putaway_rule")) {
			frm.set_df_property("apply_putaway_rule", "hidden", 1);
		}
		if (frm.get_docfield("rejected_warehouse")) {
		...
		if (frm.fields_dict["items"] && frm.fields_dict["items"].grid) {
			frm.fields_dict["items"].grid.set_column_disp("rejected_warehouse", false);
		}
