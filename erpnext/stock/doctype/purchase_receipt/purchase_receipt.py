...
def make_purchase_invoice(source_name, target_doc=None, args=None):
	...
	po_list = [d.purchase_order for d in doclist.get("items") if d.get("purchase_order")]
	if po_list:
		po_dates = frappe.get_all("Purchase Order", filters={"name": ["in", po_list]}, fields=["transaction_date"])
		if po_dates:
			max_po_date = max(frappe.utils.getdate(d.transaction_date) for d in po_dates if d.transaction_date)
			if doclist.posting_date and max_po_date and frappe.utils.getdate(doclist.posting_date) < max_po_date:
				doclist.posting_date = max_po_date
