...
frappe.ui.form.on("Purchase Order", {
	...
	refresh: function (frm) {
		frm.set_df_property("is_subcontracted", "hidden", 1);
		if (frm.get_docfield("tax_category")) {
			frm.set_df_property("tax_category", "hidden", 1);
		}
		if (frm.get_docfield("shipping_rule")) {
			frm.set_df_property("shipping_rule", "hidden", 1);
		}
		if (frm.get_docfield("incoterm")) {
		...
		// Dynamically calculate and display custom_paid_amount and custom_outstanding_amount
		if (frm.doc.name && frm.doc.docstatus === 1) {
			frappe.call({
				method: "erpnext.buying.doctype.purchase_order.purchase_order.get_po_payment_info",
				args: {
					purchase_order_name: frm.doc.name
				},
				callback: function (r) {
					if (r.message) {
						frm.doc.custom_paid_amount = r.message.custom_paid_amount;
						frm.doc.custom_outstanding_amount = r.message.custom_outstanding_amount;
			...
			frm.doc.custom_paid_amount = 0;
			frm.doc.custom_outstanding_amount = flt(frm.doc.grand_total);
			frm.refresh_field("custom_paid_amount");
			frm.refresh_field("custom_outstanding_amount");
		}
	},
