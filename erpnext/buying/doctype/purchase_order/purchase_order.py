...
class PurchaseOrder(BuyingController):
	...
	def update_payment_amounts(self):
		...
		if self.name:
			...
			if invoice_items:
				invoice_names = list(set([item.parent for item in invoice_items]))
		...
		self.custom_paid_amount = total_paid
		self.custom_outstanding_amount = flt(self.grand_total) - total_paid
...
def make_purchase_receipt(source_name, target_doc=None, args=None):
	...
	po_transaction_date = frappe.db.get_value("Purchase Order", source_name, "transaction_date")
	if doc.posting_date and po_transaction_date and frappe.utils.getdate(doc.posting_date) < frappe.utils.getdate(po_transaction_date):
		doc.posting_date = po_transaction_date
...
@frappe.whitelist()
def get_po_payment_info(purchase_order_name):
	import frappe
	from frappe.utils import flt
	total_paid = 0.0
	...
	po_grand_total = flt(frappe.db.get_value("Purchase Order", purchase_order_name, "grand_total"))
	return {
		"custom_paid_amount": total_paid,
		"custom_outstanding_amount": po_grand_total - total_paid
	}
